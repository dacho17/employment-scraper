...
async function scrapeData(page: any, url: string): Promise<JobDetails> {
    ...
    const jobTitleElement = await page.$(Constants.ARBEITNOW_JOB_TITLE_SELECTOR);
    adDetails.jobTitle = await page.evaluate(el => el.innerText, jobTitleElement);
    adDetails.jobTitle.trim();

    const companyNameElement = await page.$(Constants.ARBEITNOW_COMPANY_NAME_SELECTOR);
    adDetails.companyName = await page.evaluate(el => el.innerText, companyNameElement);
    adDetails.companyName.trim();
    
    const companyLocationElement = await page.$(Constants.ARBEITNOW_COMPANY_LOCATION_SELECTOR);
    ...
    const postedDateElement = await page.$(Constants.ARBEITNOW_POSTED_DATE_SELECTOR);
    ...
    const jobDescriptionElement = await page.$(Constants.ARBEITNOW_JOB_DESCRIPTION_SELECTOR);
